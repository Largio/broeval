...
if [[ $# -ne 5 ]]
then
echo "Usage: ./http.sh <SOURCE> <TARGET> <ITER> <SIZE> <MODE>"
echo "Times a parallel ssl secured transfer of data using https."
exit
fi
...
if [ $MODE == 'par' ]; 
then
ssh $SOURCEFINAL "/usr/bin/time -f \"%e\" bash -c \"seq $ITER | parallel -n0 curl -s http://$TARGET/$SIZE.txt >> /dev/null \""
elif [ $MODE == 'seq' ]; 
then
ssh $SOURCEFINAL "/usr/bin/time -f \"%e\" bash -c \"for i in {1..$ITER}; do curl -s http://$TARGET/$SIZE.txt > /dev/null; done\""
else
