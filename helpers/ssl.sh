...
echo "Times an ssl secured transfer of data using https."
exit
fi
...
if [ $MODE == 'par' ]; 
then
ssh $SOURCEFINAL "/usr/bin/time -f \"%e\" bash -c \"seq $ITER | parallel -n0 curl -s --insecure https://$TARGET/$SIZE.txt > /dev/null\""
elif [ $MODE == 'seq' ]; 
then
ssh $SOURCEFINAL "/usr/bin/time -f \"%e\" bash -c \"for i in {1..$ITER}; do curl -s --insecure https://$TARGET/$SIZE.txt > /dev/null; done\""
else
